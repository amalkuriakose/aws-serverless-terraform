resource "aws_api_gateway_rest_api" "edge_optimized_rest_api" {
  name = var.edge_optimized_rest_api_name
  tags = merge(
    local.common_tags,
    {
      Name = var.edge_optimized_rest_api_name
...
resource "aws_api_gateway_resource" "test_resource" {
  parent_id   = aws_api_gateway_rest_api.edge_optimized_rest_api.root_resource_id
  path_part   = "test"
  rest_api_id = aws_api_gateway_rest_api.edge_optimized_rest_api.id
}
...
resource "aws_cloudwatch_log_group" "apigw_execution_log_group" {
  name              = "API-Gateway-Execution-Logs_${aws_api_gateway_rest_api.edge_optimized_rest_api.id}/${var.edge_optimized_rest_api_stage_name}"
  retention_in_days = 7
}
...
resource "aws_api_gateway_stage" "apigw_stage" {
  ...
  rest_api_id   = aws_api_gateway_rest_api.edge_optimized_rest_api.id
  stage_name    = var.edge_optimized_rest_api_stage_name
}
